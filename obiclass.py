    ...
    # process move by move
          ...
          for item in sequence:  
            ...
            if(not item):
              ...
              data1 = np.zeros((4, 21, 21), dtype=np.uint8)
          ...
          #print(j)
          if(train < 50):
            lmdbReadWrite2.writeToDB(data, sequence, countTrain, "train_%s_%s" % (sys.argv[1], countTrain/1000000))
            countTrain = countTrain + len(sequence)
            train = train + 1;
          else:
            lmdbReadWrite2.writeToDB(data, sequence, countTest, "test_%s_%s" % (sys.argv[1], countTest/20000))
